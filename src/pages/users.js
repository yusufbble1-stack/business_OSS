      ...
      <main class="app-main">
        ...
        <div class="page-content">
          <div class="page-header animate-in">
            <div><h1>${t('user_management', {}, 'User Management')}</h1><p>${t('user_management_desc', {}, 'Manage customers, technicians and admin accounts.')}</p></div>
            <button class="btn btn-primary" id="btn-add-user">${icon('user-plus', 16)} ${t('add_user', {}, 'Add User')}</button>
          </div>
          ...
          <div class="flex gap-3 flex-wrap" style="margin-bottom:16px">
            ...
            <input type="text" id="search-users" placeholder="${t('search_users_placeholder', {}, 'Search by name or email...')}" style="max-width:300px"/>
          </div>

          <div class="card animate-in" style="padding:0;overflow-x:auto">
            <table style="min-width: 800px;">
              <thead><tr><th>${t('user', {}, 'User')}</th><th>${t('email', {}, 'Email')}</th><th>${t('role', {}, 'Role')}</th><th>${t('company', {}, 'Company')}</th><th>${t('phone', {}, 'Phone')}</th><th>${t('status')}</th><th>${t('joined', {}, 'Joined')}</th><th>${t('actions', {}, 'Actions')}</th></tr></thead>
  ...
  return users.map(u => `
    <tr>
      ...
      <td><span style="color:${u.is_active ? 'var(--status-completed)' : 'var(--status-cancelled)'};font-size:var(--text-xs);font-weight:600;display:flex;align-items:center;gap:4px">
        <span style="width:6px;height:6px;border-radius:50%;background:currentColor"></span>${u.is_active ? t('active', {}, 'Active') : t('inactive', {}, 'Inactive')}
      </span></td>
...
function bindUserEvents() {
  ...
  document.getElementById('btn-add-user')?.addEventListener('click', () => {
    showModal(t('add_new_user', {}, 'Add New User'), `
      <div class="form-group"><label>${t('full_name')} *</label><input type="text" id="new-name" placeholder="John Doe"/></div>
      <div class="form-group"><label>${t('email', {}, 'Email')} *</label><input type="email" id="new-email" placeholder="user@email.com"/></div>
      <div class="form-row">
        <div class="form-group"><label>${t('role', {}, 'Role')}</label><select id="new-role"><option value="customer">${t('customer', {}, 'Customer')}</option><option value="technician">${t('technician')}</option><option value="admin">Admin</option></select></div>
        <div class="form-group"><label>${t('phone', {}, 'Phone')}</label><input type="tel" id="new-phone" placeholder="+33 6 ..."/></div>
      </div>
      <div class="form-group"><label>${t('company', {}, 'Company')}</label><input type="text" id="new-company" placeholder="Company name"/></div>
    `, [{
      id: 'modal-create-user', label: `${icon('plus', 14)} ${t('create_user', {}, 'Create User')}`, class: 'btn-primary',
      onClick: async (_, close) => {
        const name = document.getElementById('new-name')?.value?.trim();
        const email = document.getElementById('new-email')?.value?.trim();
        if (!name || !email) { showToast(t('name_email_required', {}, 'Name and email are required'), 'error'); return; }
        await createProfile({
        ...
        showToast(t('user_created_success', { name }, 'User "{name}" created successfully'), 'success');
        close();
        renderUsersPage();
      },
    }]);
  ...
  document.querySelectorAll('.toggle-user-btn').forEach(btn => {
    btn.addEventListener('click', async () => {
      ...
      if (profile) {
        const statusMsg = profile.is_active ? t('profile_activated_toast', { name: profile.full_name }, '{name} activated') : t('profile_deactivated_toast', { name: profile.full_name }, '{name} deactivated');
  ...
  document.querySelectorAll('.edit-user-btn').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      if (!u) return;
      showModal(t('edit_user_title', { name: u.full_name }, 'Edit {name}'), `
        <div class="form-group"><label>${t('full_name')}</label><input type="text" id="edit-name" value="${u.full_name}"/></div>
        <div class="form-row">
          <div class="form-group"><label>${t('phone', {}, 'Phone')}</label><input type="tel" id="edit-phone" value="${u.phone || ''}"/></div>
          <div class="form-group"><label>${t('company', {}, 'Company')}</label><input type="text" id="edit-company" value="${u.company_name || ''}"/></div>
        </div>
      `, [{
        id: 'modal-save-user', label: `${icon('save', 14)} ${t('save', {}, 'Save')}`, class: 'btn-primary',
        onClick: async (_, close) => {
          ...
          showToast(t('user_updated_toast', {}, 'User updated'), 'success');
          close();
          renderUsersPage();
        },
      }]);
